...
arrow::Status WriteToFile(std::shared_ptr<arrow::Table> table,
  ...
  //      WriterProperties::Builder().disable_dictionary()->compression(parquet::Compression::UNCOMPRESSED)->encoding(kSrcIndexCol, parquet::Encoding::RLE)->encoding(kDstIndexCol, parquet::Encoding::DELTA_BINARY_PACKED_FOR_BIT_MAP)->build();
  std::shared_ptr<WriterProperties> props =
...
int main (int argc, char* argv[]) {
  ...
  auto edge_table = read_csv_to_arrow_table(edge_source_file)->SelectColumns({0, 1}).ValueOrDie();
  auto table = SortKeys(edge_table);
  std::cout << "table: " << table->ToString() << std::endl;
  WriteToFileBaseLine(table, path_to_file);
  // auto offset = getOffsetTable(edge_table, kSrcIndexCol, 41652230);
  auto offset = getOffsetTable(edge_table, kSrcIndexCol, 58655849);
  // WriteOffsetToFile(offset, path_to_file + "-offset");
  return 0;
}
