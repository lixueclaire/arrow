...
void ReadBitMap(const std::string& path_to_file, const int64_t& offset, const int64_t& length, uint64_t* bit_map) {
  ...
  auto file_metadata = reader_->metadata();
  int i = 0;
  int col_id = file_metadata->schema()->ColumnIndex("_graphArDstIndex");
  ...
  int64_t values_read = 0;
  auto col_reader = std::static_pointer_cast<parquet::Int64Reader>(reader_->RowGroup(i++)->Column(col_id));
  ...
  while (total_values_remaining > 0) {
    col_reader = std::static_pointer_cast<parquet::Int64Reader>(reader_->RowGroup(i++)->Column(col_id));
    while (col_reader->HasNext() && total_values_remaining > 0) {
      col_reader->ReadBatch(total_values_remaining, bit_map, &values_read);
      total_values_remaining -= values_read;
    }
...
std::shared_ptr<arrow::Table> SelectOptimizePerson(const std::string& path_to_file, uint64_t* bit_map) {
  arrow::Int64Builder _id_builder;
  arrow::StringBuilder _first_name_builder;
  arrow::StringBuilder _last_name_builder;
  arrow::Int64Builder _index_builder;
  ...
  int col_id = file_metadata->schema()->ColumnIndex("personId");
  int col_id2 = file_metadata->schema()->ColumnIndex("firstName");
  int col_id3 = file_metadata->schema()->ColumnIndex("lastName");
  int col_id4 = file_metadata->schema()->ColumnIndex("personIndex");
  std::cout << "file metadata: " << file_metadata->schema()->ToString() << std::endl;
  int64_t value;
  for (int rg = 0; rg < row_group_count; ++rg) {
    ...
    auto index_reader = std::static_pointer_cast<parquet::Int64Reader>(row_group_reader->Column(col_id4));
    ...
    for (int64_t i = 0; i < num_row_rg; i++) {
      ...
      if ((bit_map[index >> 6] & (1UL << (index & 63)))) {
        ...
        index_reader->ReadBatch(1, nullptr, nullptr, &value, &values_read);
        _index_builder.Append(value);
        last_i = i + 1;
        count++;
      }
      index++;
  ...
  std::vector<std::shared_ptr<arrow::Field>> schema_vector = {
      ...
      arrow::field("personIndex", arrow::int64())};
  auto schema = std::make_shared<arrow::Schema>(schema_vector);
  return arrow::Table::Make(schema, {_id_array, _first_name_array, _last_name_array, _index_array});
}

std::shared_ptr<arrow::Table> SelectOptimizeComment(const std::string& path_to_file, uint64_t* bit_map) {
  arrow::Int64Builder _id_builder, _creation_date_builder, _index_builder;
  arrow::StringBuilder _content_builder;
  ...
  std::shared_ptr<arrow::Array> _id_array, _content_array, _index_array, _creation_date_array;
  _id_builder.Finish(&_id_array);
  _content_builder.Finish(&_content_array);
  _index_builder.Finish(&_index_array);
  _creation_date_builder.Finish(&_creation_date_array);
  std::vector<std::shared_ptr<arrow::Field>> schema_vector = {
      arrow::field("commentId", arrow::int64()),
      arrow::field("content", arrow::utf8()),
      arrow::field("creationDate", arrow::int64()),
      arrow::field("personIndex", arrow::int64())};
  auto schema = std::make_shared<arrow::Schema>(schema_vector);
  return arrow::Table::Make(schema, {_id_array, _content_array, _creation_date_array, _index_array});
}
...
void IC8_GRAPHAR(
  ...
  uint64_t* bit_map_person = new uint64_t[vertex_num_person / 64 + 1];
  ...
  memset(bit_map_person, 0, sizeof(uint64_t) * (vertex_num_person / 64 + 1));
  ...
  std::cout << "read post reply" << std::endl;
  ...
  std::cout << "read comment reply" << std::endl;
  std::unique_ptr<parquet::ParquetFileReader> person_offset_reader =
    parquet::ParquetFileReader::OpenFile(comment_has_creator_person_path_file + "_alter-offset");
  std::unique_ptr<parquet::ParquetFileReader> person_delta_reader =
    parquet::ParquetFileReader::OpenFile(comment_has_creator_person_path_file + "_alter-delta");
  for (int64_t i = 0; i < comment_length; ++i) {
    bit_map_comment_i = bit_map[i];
    index = i * 64;
    while (bit_map_comment_i) {
      bit_offset = __builtin_ctzll(bit_map_comment_i);
      index += bit_offset;
      getOffset(person_offset_reader, index, offset, length);
      if (length > 0) {
        ReadBitMap(person_delta_reader, offset, length, bit_map_person);
      }
      bit_map_comment_i = (bit_map_comment_i >> bit_offset) >> 1;
      ++index;
    }
  ...
  auto comment_property_table = SelectOptimizeComment(comment_path_file, bit_map);

  auto person_property_table = SelectOptimizePerson(person_path_file, bit_map_person);

  std::cout << "read property, row num: " << comment_property_table->num_rows() << " " << person_property_table->num_rows() << std::endl;
  
  auto result = DoHashJoin(comment_property_table, person_property_table, "personIndex", "personIndex", {"commentId", "creationDate", "content"}, {"personId", "firstName", "lastName"});
  std::cout << "hash join";
  ...
  //   }
  // }
  std::cout << "result row num: " << result->num_rows() << std::endl;
  auto sorted_table = SortOperation(result, "creationDate", "commentId")->Slice(0, 20);
  auto run_time = 1000.0 * (clock() - run_start) / CLOCKS_PER_SEC;
  std::cout << "table: " << sorted_table->ToString() << std::endl;
